...
class User(db.Model, UserMixin):
    ...
    username: so.Mapped[str] = so.mapped_column(
        sa.String(25), unique=True, nullable=False
    )
    email: so.Mapped[str] = so.mapped_column(
        sa.String(125), unique=True, nullable=False
    )
    image_file: so.Mapped[str] = so.mapped_column(
        sa.String(20), nullable=False, default="default.jpg"
    )
    bio = db.Column(db.Text, nullable=True)
    password: so.Mapped[str] = so.mapped_column(sa.String(60), nullable=False)
    lessons: so.WriteOnlyMapped[List["Lesson"]] = so.relationship(
        "Lesson", back_populates="author", lazy=True
    )
    def __repr__(self):
...
class Course(db.Model):
    ...
    icon: so.Mapped[str] = so.mapped_column(
        sa.String(20), nullable=False, default="default_icon.jpg"
    )

    lessons: so.WriteOnlyMapped[List[Lesson]] = so.relationship(
        "Lesson", back_populates="course_name", lazy=True
    )
